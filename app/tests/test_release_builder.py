...
def test_release_builder_is_deterministic_and_excludes_user_data(tmp_path):
    ...
    first = build_release(app, tmp_path / "one", version="v74.0.1")
    second = build_release(app, tmp_path / "two", version="v74.0.1")
...
def test_manifest_hashes_match_update_zip(tmp_path):
    app = _fake_app(tmp_path)
    result = build_release(app, tmp_path / "out", version="v74.0.1")
    manifest = json.loads(result.manifest_bytes)

    assert manifest["version"] == "v74.0.1"
