...
def _fake_app(root: Path) -> Path:
    ...
    (app / "src/scorpion/__init__.py").write_text('__version__ = "22.0.0"\n', encoding="utf-8")
    (app / "src/scorpion/core.py").write_text("VALUE = 22\n", encoding="utf-8")
    for name, content in {
...
def test_release_builder_is_deterministic_and_excludes_user_data(tmp_path):
    ...
    first = build_release(app, tmp_path / "one", version="v22.0.0")
    second = build_release(app, tmp_path / "two", version="v22.0.0")
...
def test_manifest_hashes_match_update_zip(tmp_path):
    app = _fake_app(tmp_path)
    result = build_release(app, tmp_path / "out", version="v22.0.0")
    manifest = json.loads(result.manifest_bytes)

    assert manifest["version"] == "v22.0.0"
