...
def build_release(app_root: Path | str, output_dir: Path | str, *, version: str) -> ReleaseBuild:
    ...
    if not version.startswith("v") or version != "v23.0.0":
        raise ValueError("MK23 release version must be v23.0.0")
    ...
    full_zip = output_dir / "SCORPION_MK23.zip"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build deterministic Scorpion MK23 release assets.")
    parser.add_argument("--app-root", default=".")
    parser.add_argument("--out", required=True)
    parser.add_argument("--version", default="v23.0.0")
